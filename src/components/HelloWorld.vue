...
export default {
  ...
  computed: {
    scheduleType() {
      // Convert tab index to schedule type
  ...
  watch: {
    scheduleType: {
      handler(newValue) {
        // Initialize appropriate fields when schedule type changes
        switch(newValue) {
          case 'specific':
            ...
            break;
          case 'daily':
            if (this.dailySchedule.times.length === 0) {
              this.addDailyTime();
            }
            break;
          case 'weekly':
            if (this.weeklySchedule.times.length === 0) {
              this.addWeeklyTime();
            }
            break;
        }
